-- Migration 0054: Update view v_receipt_missing_status VAT warning threshold
-- Date: 2025-12-27
-- Purpose:
-- Avoid false warning when VAT totals are present but zero (VAT-exempt receipts).
-- Change vat_present_but_gross_equals_net to trigger only when VAT totals > 0.00.
...
SELECT
  ...
  CONCAT_WS(',',
    ...
    IF(
      ...
      AND (
        COALESCE(uf.total_vat_25, 0) > 0
        OR COALESCE(uf.total_vat_12, 0) > 0
        OR COALESCE(uf.total_vat_6, 0) > 0
      ),
