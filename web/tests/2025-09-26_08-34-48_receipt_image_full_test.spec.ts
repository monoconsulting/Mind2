...
test.describe('Receipt Image Orientation - Complete Test', () => {
  ...
  test.beforeEach(async ({ page }) => {
    // Navigate to receipts page with larger viewport
    await page.setViewportSize({ width: 1900, height: 1200 });

    // Go directly to receipts page
    await page.goto('http://localhost:3000/receipts');

    // Wait for page to fully load
    await page.waitForLoadState('networkidle');

    // Wait for receipts table
    await page.waitForSelector('table.table-dark', { timeout: 15000 });
