...
def get_workflow_stage(workflow_run_id: int, stage_key: str) -> dict[str, Any] | None:
    ...
    try:
        with db_cursor() as cur:
            cur.execute(
                "SELECT id, stage_key, status, started_at, finished_at, message "
                "FROM workflow_stage_runs WHERE workflow_run_id = %s AND stage_key = %s",
...
def mark_stage(
    ...
    """Insert/update workflow_stage_runs and bump workflow_runs current status."""
    if db_cursor is None:
        return False
...
def begin_import_stage(workflow_run_id: Optional[int], stage_base: str, *, message: str | None = None) -> None:
    """Log the start of a high-level import stage."""
    ...
    if stage_base in ("src_portal", "src_ftp", "src_fc"):
        ...
        if wfr and wfr.get("file_id"):
            try:
                ...
                logger.debug("Unable to set ai_status='processing' for %s", wfr.get("file_id"))
            else:
                logger.info(
...
def complete_import_stage(
    ...
    *,
    ...
    """Log completion of an import stage and annotate boundary markers."""
    ...
    if stage_base == "finalize_ok" and success:
        ...
        if wfr and wfr.get("file_id"):
            try:
                ...
                logger.debug("Unable to set ai_status='completed' for %s", wfr.get("file_id"))
            else:
                logger.info(
