...
def _wire_legacy_proxies() -> None:
    legacy_module = sys.modules.get("services.tasks.legacy")
    ...
    for attr in proxy_names:
        if hasattr(legacy_module, attr):
            setattr(legacy_module, attr, _make_tasks_proxy(attr))


_wire_legacy_proxies()
