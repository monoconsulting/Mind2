...
def test_resume_statement_workflow(
    ...
    # Mock db_cursor in statements.py (it uses db_cursor to fetch workflow_run_id)
    with patch("api.reconciliation_firstcard.routes.statements.db_cursor") as mock_db_cursor:
        ...
        # Mock fetchone for "SELECT workflow_run_id..."
        # It returns (workflow_run_id, status, current_stage)
        ...
        mock_coordinator.begin_fc_import_stage.assert_called()
        args, kwargs = mock_coordinator.begin_fc_import_stage.call_args
        assert args[0] == 123
        assert args[1] == "resume_dispatch"
        
        mock_coordinator.complete_fc_import_stage.assert_called_with(
            123, "resume_dispatch", success=True, message="Resume dispatched"
        )
...
def test_restart_statement_workflow(
    mock_write_metadata,
    mock_log_event,
    mock_transition_doc,
    mock_transition_proc,
    ...
    with patch("api.reconciliation_firstcard.routes.statements.db_cursor") as mock_db_cursor:
        ...
        # Mock fetchone for metadata check
        ...
        args, kwargs = mock_coordinator.begin_fc_import_stage.call_args
        assert args[0] == 456
        assert args[1] == "restart_dispatch"
        
        mock_coordinator.complete_fc_import_stage.assert_called_with(
            456, "restart_dispatch", success=True, message="Restart dispatched"
        )
