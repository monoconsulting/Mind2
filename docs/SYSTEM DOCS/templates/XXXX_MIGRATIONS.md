...
# XXXX — Migrations Policy (Template)

- Tooling: choose one (Flyway, Liquibase, Alembic, Prisma, Knex)
- Directory: `db/migrations`
  - Naming: `V0001__init.sql`, `V0002__add_items.sql`
- Reversibility: every migration must have rollback plan
- CI: apply migrations before integration tests
- Deploy order: DB first (migrate), then services
