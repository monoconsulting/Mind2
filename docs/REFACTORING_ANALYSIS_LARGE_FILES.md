# Analys av Stora Filer - Refactoring-mojligheter
...
Analysen har identifierat flera mycket stora filer som kraftigt overskrider rekommenderade storleksgranser och som innehaller for mycket ansvar (Single Responsibility Principle violations). Dessa filer ar svara att underhalla, testa och forsta.

**Kritiska fynd:**
- Minst atta filer overstiger 1 000 rader kod
- Den storsta filen ar nu 2 418 rader (`backend/src/api/reconciliation_firstcard.py`)
- Celery-monoliten `backend/src/services/tasks.py` ersattes 2025-11-09 av ett paket, men `creditcard_tasks.py` (1 014 rader) och `workflow_tasks.py` (762 rader) ar fortfarande for stora
- Flera filer bryter mot Single Responsibility Principle
- Hog komplexitet med manga funktioner i samma fil
- Risk for kod-dublicering och svarigheter vid testning
- Pytest-kommandot `pytest backend/tests/unit/test_tasks_invoice_pipeline.py` faller just nu pa `ModuleNotFoundError: celery` (synk med 25-11-09_Worklog)

---

## Senaste ändringar (2025-11-09)

- Importerade paketstrukturen från PR #65 (`backend/src/services/tasks/**`) och tog bort monoliten `services/tasks.py`.
- Lagt till `legacy.py` för att exponera historiska Celery-tasks tills övriga konsumenter flyttats.
- Pytest på `backend/tests/unit/test_tasks_invoice_pipeline.py` stoppar p.g.a. att Celery inte hittas i nuvarande tolk (global interpreter ser inte user site-packages trots `pip install celery`).
- Inga regressions-tester har passerat ännu; refactoreringen ligger endast i working tree (ej committad/deployerad).

---

## TOP 10 Kritiska Filer som Kraver Omstrukturering

### 1. ?? STATUS: `backend/src/services/tasks/` (tidigare `tasks.py`)
**Status 2025-11-09 (synk med 25-11-09_Worklog.md):**
- Monoliten `backend/src/services/tasks.py` (cirka 4 344 rader) ar borttagen lokalt.
- Paketet `backend/src/services/tasks/` bestar nu av moduluppdelningen som foreslogs nedan.
- `legacy.py` exponerar gamla Celery-entrypoints (`process_ocr`, `process_ai_pipeline`, etc.) for bakatkompatibilitet.

**Nuvarande modulstorlekar:**
| Fil | Rader | Kommentar |
|---|---|---|
| `creditcard_tasks.py` | 1 014 | Blandar matching, AI6 och accounting-proposal; maste brytas upp |
| `workflow_tasks.py` | 762 | Innehalter dispatch + workflow chains; bor ersattas av koordinator + mindre moduler |
| `ai_pipeline_tasks.py` | 533 | Hanterar AI-steg 1-4; fortsatt stor men homogen |
| `invoice_tasks.py` | 521 | Invoices + metadata; bor delas i service + tasklager |
| `ocr_tasks.py` | 408 | OCR-tasks; acceptabel storlek tills vidare |
| `legacy.py` | 273 | Tillfallig shim som bor fasas ut nar alla imports pekar pa nya moduler |

**Identifierade risker efter refactor:**
- `creditcard_tasks.py` och `workflow_tasks.py` ar fortfarande for stora och bryter mot SRP.
- Celery-beroendet saknas i testmiljon; pytest faller trots `pip install celery` (global interpreter ser inte user site-packages).
- `legacy.py` introducerar dubbla kodvagar; risk att nya features hamnar dar i stallet for i modulerna.
- Ingen verifierad testkorning efter split -> risk for regressions i OCR/AI-pipelines.

**Nasta steg (akuta atgarder):**
1. Lagga till Celery i projektets officiella requirements/poetry och kor `pytest backend/tests/unit/test_tasks_invoice_pipeline.py` i en miljo som hittar paketet (t.ex. Docker compose).
2. Dela `creditcard_tasks.py` i minst tva filer (`matching_tasks.py`, `card_ingest_tasks.py`) och flytta accounting-proposal helpers till en service.
3. Flytta workflow-dispatch och chain-byggare till en dedikerad `workflow_coordinator.py` + sma workflow-moduler; lat `workflow_tasks.py` enbart registrera Celery-tasks.
4. Planera migrering fran `services.tasks`-importer till nya modulvagar och ta bort `legacy.py` nar samtliga scripts/testriggar ar uppdaterade.

**Implementerad struktur (2025-11-09):**
```
services/tasks/
├── __init__.py
├── ocr_tasks.py
...
2. **`services/tasks`-paketet** (uppfoljning efter refactor)

   - Vecka 3 (klart 2025-11-09): Moduluppdelning + legacy-shim pa plats

   - Vecka 4: Stabilisera & testa - fixa Celery-beroendet och kor `pytest backend/tests/unit/test_tasks_invoice_pipeline.py`

   - Vecka 4: Dela `creditcard_tasks.py` och `workflow_tasks.py` i mindre filer + extrahera services

   - Vecka 5: Migrera externa imports fran `services.tasks` till nya moduler och ta bort `legacy.py`
...
Efter Workflow Coordinator är implementerad behöver vi omedelbart stabilisera `services/tasks`-paketet (säker Celery-miljö, uppdelning av `creditcard_tasks.py`/`workflow_tasks.py`, avveckla `legacy.py`) innan vi går vidare med resten av planen.
