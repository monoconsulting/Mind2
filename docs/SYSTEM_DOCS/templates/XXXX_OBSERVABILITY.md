...
# XXXX — Observability (Template)

- Logging: JSON lines with UTC timestamps; include `requestId` and route
- Metrics: Prometheus `/metrics` per service (reqs, latency, errors)
- Tracing: OpenTelemetry; propagate `X-Request-Id`/`X-Correlation-Id`
- Health: `/v1/health` (liveness), `/v1/ready` (readiness)
