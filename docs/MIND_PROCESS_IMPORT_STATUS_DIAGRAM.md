# Nu Import- och Matchningsflöde

**Version:** 2.0 (Uppdaterad 2025-11-08)

Detta diagram visar end-to-end-flödet för kvitton (WF1_RECEIPT) och FirstCard-fakturor (WF3_FIRSTCARD_INVOICE), inklusive källor, ingest, OCR/AI-steg, automatchning, utfall OCH återuppta-funktionalitet. Används som referens för felsökning och vidareutveckling.
...
flowchart TD
  ...
  subgraph RESUME["Återuppta/Omstart"]
    direction TB
    resume_dispatch_start["resume_dispatch startad ⏳"]
    resume_dispatch["Återupptar bearbetning 🔄\n(Process.jsx 'Återuppta' → /ai/api/ingest/process/{id}/resume)\n(CompanyCard.jsx 'Återuppta' → /ai/api/reconciliation/firstcard/statements/{id}/resume)"]
    resume_dispatch_end["resume_dispatch avslutad ✅"]

    restart_dispatch_start["restart_dispatch startad ⏳"]
    restart_dispatch["Omstartar från början 🔄\n(CompanyCard.jsx 'Återuppta' på completed → /ai/api/reconciliation/firstcard/statements/{id}/restart)"]
    restart_dispatch_end["restart_dispatch avslutad ✅"]
  end
  ...
  finalize_fail_start --> finalize_fail --> finalize_fail_end

  %% ---------- ÅTERUPPTA/OMSTART FLÖDEN ----------
  %% Återuppta från KLAR (när användare trycker "Återuppta" på slutförd fil)
  KLAR -.->|Återuppta| resume_dispatch_start

  %% Återuppta från misslyckad eller manuell granskning
  finalize_fail_end -.->|Återuppta| resume_dispatch_start
  manual_review -.->|Återuppta| resume_dispatch_start

  %% Resume-dispatch flöde
  resume_dispatch_start --> resume_dispatch --> resume_dispatch_end

  %% Efter resume_dispatch: fortsätt workflow (vanligtvis till matchning)
  resume_dispatch_end -->|Fortsätt WF1/WF3| ai5_start

  %% Omstart från början (bara för FC-fakturor)
  KLAR -.->|Omstart FC| restart_dispatch_start
  finalize_fail_end -.->|Omstart FC| restart_dispatch_start

  %% Restart-dispatch flöde
  restart_dispatch_start --> restart_dispatch --> restart_dispatch_end

  %% Efter restart_dispatch: börja om från början av FC-workflow
  restart_dispatch_end -->|Starta om WF3| fc_create_start

  %% ---------- Klassapplicering ----------
  class src_portal_start,src_ftp_start,src_fc_start,ingest_store_start,ingest_wf1_start,fc_create_start,fc_ocr_start,fc_parse_start,fc_ready_start,detect_type_start,r_ocr_start,r_ai3_start,r_ai4_start,r_persist_start,r_queue_match_start,ai5_start,m_link_start,m_unmatched_start,finalize_ok_start,finalize_fail_start,resume_dispatch_start,restart_dispatch_start start;
  class src_portal_end,src_ftp_end,src_fc_end,ingest_store_end,ingest_wf1_end,fc_create_end,fc_ocr_end,fc_parse_end,fc_ready_end,detect_type_end,r_ocr_end,r_ai3_end,r_ai4_end,r_persist_end,r_queue_match_end,ai5_end,m_link_end,m_unmatched_end,finalize_ok_end,finalize_fail_end,resume_dispatch_end,restart_dispatch_end,KLAR done;
